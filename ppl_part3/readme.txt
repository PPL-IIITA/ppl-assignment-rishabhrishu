 ...
 * Documentation generation script is included in build.xml file, so running ant will generate javadoc
 ...
 * To build the project, Apache Ant is required.
  ...
  * First of all, we need to run build_runTestCase.sh to generate test case.
	...
	If we dont want to use the sh file, or while using environments other than linux, or in case script doesn't work, 
	...
	For eg., to recompile and run ques3 , we will type following on command prompt(shown by linux:~$ ) ->
		linux:~$ ppl-assign-master/ppl_part3$ cd ques3/
		linux:~$ ppl-assign-master/ppl_part3/ques3$ ant
		linux:~$ ppl-assign-master/ppl_part3/ques3$ cd dist/
		linux:~$ ppl-assign-master/ppl_part3/ques3/dist$ java -jar ques3.jar
			//program will execute
		linux:~$ ppl-assign-master/ppl_part3/ques3/dist$ cd ../.. 
		linux:~$ ppl-assign-master/ppl_part3$ 
